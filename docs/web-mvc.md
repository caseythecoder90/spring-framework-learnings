...
```bash
./tools/fetch-spring-sources.sh spring-web spring-webmvc
```

Now you can grep them, which is faster than any IDE search:

```bash
grep -rn "class DispatcherServlet" .spring-sources/
```

**Then walk the code path shown earlier in this note, in order.** It is not a list of classes to read in full. Each stop names
...
**You have understood this when you can say, without looking:** where the filter chain ends and
`DispatcherServlet` begins, and why that boundary decides your error handling.
