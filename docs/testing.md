...
```bash
./tools/fetch-spring-sources.sh spring-test
```

Now you can grep them, which is faster than any IDE search:

```bash
grep -rn "class DefaultContextCache" .spring-sources/
```
...
<!-- widget:path:context-cache -->

**You have understood this when you can say, without looking:** everything that goes into the
context cache key.
