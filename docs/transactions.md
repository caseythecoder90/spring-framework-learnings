...
```bash
./tools/fetch-spring-sources.sh spring-tx
```

Now you can grep them, which is faster than any IDE search:

```bash
grep -rn "class ProxyTransactionManagementConfiguration" .spring-sources/
```
...
**You have understood this when you can say, without looking:** which single method contains all
seven propagation modes.
